      ...
      ingress_rules = [
        {
          ...
          to_port     = 443,
          protocol    = "tcp",
          cidr_blocks = ["10.1.1.0/24", "10.1.176.0/23", "172.16.0.0/22", "10.1.0.0/16"]
        },
      ]
      egress_rules = [
        {
...
module "endpoints" {
  ...
  vpc_endpoints = [
...

    {
      ...
      vpc_endpoint_type  = "Interface"
      security_group_ids = [for tag, id in module.sg.ids : id if tag == "tradingbot-${var.environment}-ec2-endpoint-sg"]
      subnet_ids         = [for tag, id in module.subnets.ids : id if contains(["tradingbot-${var.environment}-eks-private-1a-1", "tradingbot-${var.environment}-eks-private-1b-1"], tag)]
      tag                = "ec2"
    },
    {
      ...
      vpc_endpoint_type  = "Interface"
      security_group_ids = [for tag, id in module.sg.ids : id if tag == "tradingbot-${var.environment}-ecr-endpoint-sg"]
      subnet_ids         = [for tag, id in module.subnets.ids : id if contains(["tradingbot-${var.environment}-eks-private-1a-1", "tradingbot-${var.environment}-eks-private-1b-1"], tag)]
      tag                = "ecr-api"
    },
    {
    ...
    },
    {
      ...
      vpc_endpoint_type  = "Interface"
      security_group_ids = [for tag, id in module.sg.ids : id if tag == "tradingbot-${var.environment}-eks-endpoint-sg"]
      subnet_ids         = [for tag, id in module.subnets.ids : id if contains(["tradingbot-${var.environment}-eks-private-1a-1", "tradingbot-${var.environment}-eks-private-1b-1"], tag)]
      tag                = "eks"
    },
    {
